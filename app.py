        ...
        with st.spinner("Processing molecules..."):
            try:
                ...
                if len(results) > 0:
                    ...
                    with col1:
                        st.metric("Mean Similarity", f"{results['similarity'].mean():.3f}")
                    with col2:
                        st.metric("Max Similarity", f"{results['similarity'].max():.3f}")
                    with col3:
                        st.metric("Min Similarity", f"{results['similarity'].min():.3f}")
                    with col4:
                    ...
                    for i, (_, row) in enumerate(results.iterrows()):
                        col_idx = i % 3
                        with cols[col_idx]:
                            st.markdown(f"**{i+1}. Similarity: {row['similarity']:.3f}**")
                            mol = row['mols']
                            if mol:
                            ...
                            st.write(f"MW: {row['MolWt']:.1f} | LogP: {row['LogP']:.2f} | TPSA: {row['TPSA']:.1f}")
                            
                            st.markdown("---")
                    
                    # Download results
                    csv = results[['smiles', 'similarity', 'MolWt', 'LogP', 'TPSA']].to_csv(index=False)
                    st.download_button(
